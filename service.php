		...
		if (!empty($res[0]->filter)) {
			...
			if (empty($check)) {
				return $response->setTemplate('message.ejs', [
					...
					'icon' => 'thumb_down',
					'text' => "Esta es una encuesta cerrada a la cual fue invitada solo un grupo específico de usuarios. Si recibió una invitación y este mensaje le sale por error, por favor escríbanos al soporte.",
